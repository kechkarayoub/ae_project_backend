(function($) {
    setTimeout(function(){
        var $ = $ || django.jQuery;
        $(".field-emails_image").on('change', 'input[type=file]', function(event){
            var self = this;
            ...
            for (var i = 0, f; f = files[i]; i++){
                ...
                reader.onload = (function(){
                    return function(e){
                        var img = $(self).closest(".module").find('.field-get_emails_image_preview img');
                        if(img.length){
                            $(img).attr("src", e.target.result);
                        }
                        else{
                            $(self).closest(".module").find(".field-get_emails_image_preview .readonly").html(
